...
public interface CalendarRepository extends JpaRepository<CalendarEntity, Long> {
    @Query(value = "SELECT DATE(c.date) as date, " +
            ...
            "FROM calendar c " +
            "WHERE DATE(c.date) BETWEEN :startOfMonth AND :endOfMonth AND c.userid = :userid " +
            "GROUP BY DATE(c.date)", nativeQuery = true)
    ...
    @Query(value = "SELECT c.subcategory, " +
            ...
            "(SELECT SUM(c2.price) FROM calendar c2 WHERE c2.category = 'MINUS' AND c2.userid = :userid AND c2.date BETWEEN :startOfMonth AND :endOfMonth), 2) as percentage " +
            "FROM calendar c " +
